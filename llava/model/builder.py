...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            model, lora_key_value = model.merge_and_unload()
            print(type(model))
    ...
    if 'llava' in model_name.lower() and 'lora' in model_name.lower() and model_base is not None:
        return tokenizer, model, image_processor, context_len, lora_key_value
    else:
